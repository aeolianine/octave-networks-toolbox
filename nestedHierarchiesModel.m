...
function eL = nestedHierarchiesModel(N, L, G, kbar, rho)
    ...
    if length(G) ~= L
        printf('The number of levels do not match. length(G) should be L');
        return;
    end

    for x = 2:L

        if G(x) / G(x - 1) ~= ceil(G(x) / G(x - 1));
            printf('number of groups not an integer at level %2i\n', x);
            return;
        end
    ...
    if nargin < 5
        rho = kbar / (G(L) - 1) - 1 + 0.05;
        end; % set to lower bound

        if rho < kbar / (G(L) - 1) - 1;
            printf('rho is below its theoretical lower bound, given kbar and G(L)\n');
            return;
        end
        ...
        for ii = 1:N
            belongsto{ii} = zeros(1, L);
        end % the level 1, 2, 3,... groups ii belongs to
