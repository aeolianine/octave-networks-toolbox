...
function [] = radialPlot(adj, i0)
    ...
    if nargin==1
        I=sortNodesBySumNeighborDegrees(adj); 
        i0=I(1); 
    end
    ...
    for k=length(circles)-1:-1:1
        ...
        for nei=1:length(L{node})
            nei = L{node}(nei);
            if sum(find(circles{k+1}==nei))>0
                descendants{node}=descendants{node}+descendants{nei}+1; 
            end
        end
      end
    end
    ...
    for i=1:length(L{i0})
        descendants{i0}=descendants{i0} + descendants{L{i0}(i)}; 
    end
    ...
    for i=1:size(adj,1)
        ...
        plot(x,y,'.','Color',map(mod(5*circle{i},length(map))+1,:)); 
        hold off; 
        hold on;
    end
